...
def parse_args():
    ...
    ap.add_argument("--view", choices=list(VIEW_BY_NAME), default="sim_state",
                    help="render view (sim_state = whole map; bev/persp similar size for nuplan)")
    ap.add_argument("--no-render", action="store_true",
                    help="skip render calls, just capture attention (fast: ~2 min vs ~22 min)")
    ap.add_argument("--map-seed", type=int, default=42,
                    help="env map_seed → pins which maps are loaded (reproducible across runs)")
...
def main():
    ...
    try:
        ...
        lines.append("")
        lines.append(f"longest-active: env_id={long_idx}  map={map_ids[long_idx]}  "
