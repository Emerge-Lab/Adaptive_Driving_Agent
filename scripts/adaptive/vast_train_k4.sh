#!/bin/bash
# Task C (handoff §5): extra seeds for the headline k=4 cells,
...
GPU=$1; SEED=$2; MAX_MINIBATCH_SIZE=${3:-8040}; TOTAL_TIMESTEPS=${4:-3000000000}; EVAL_INTERVAL=${5:-10}; VEC=${6:-16}  # must divide 50*HORIZON=40200, multiple of 804: {8040,20100,40200}
export CUDA_VISIBLE_DEVICES=$GPU
export WANDB_MODE=${WANDB_MODE:-online}
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
export PUFFER_TRANSFORMER_LEGACY_EVAL=1

PARTNER_ID=${PARTNER_ID:-2e029h15}
ENTROPY_UB=${ENTROPY_UB:-0.10}
K_SCENARIOS=4
SCENARIO_LENGTH=201
HORIZON=$((K_SCENARIOS * SCENARIO_LENGTH))   # 804

TAG="ada_k4_gb3_legacy_eval_fix"

echo "[vast_k4] gpu=$GPU seed=$SEED horizon=$HORIZON max_minibatch=$MAX_MINIBATCH_SIZE steps=$TOTAL_TIMESTEPS tag=$TAG"
