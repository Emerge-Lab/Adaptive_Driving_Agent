#!/bin/bash
# Task A (handoff §5): k=5 STANDARD-recipe rerun — resolves the confounded
# "k=5 flat" anomaly (original k=5 cells ran the memsafe variant).
# Identical to cluster_hiddensize_ablation.sh recipe (default h=256) except:
#   - K_SCENARIOS=6 (HORIZON=1005)
...
GPU=$1; SEED=$2; MAX_MINIBATCH_SIZE=${3:-6030}; TOTAL_TIMESTEPS=${4:-3000000000}; EVAL_INTERVAL=${5:-10}; VEC=${6:-16}  # must divide 50*HORIZON=60300, mult of 1206: {6030,12060,30150}
...
K_SCENARIOS=6
SCENARIO_LENGTH=201
HORIZON=$((K_SCENARIOS * SCENARIO_LENGTH))   # 1206

TAG="k6_standard_recipe_e010"

echo "[vast_k6] gpu=$GPU seed=$SEED horizon=$HORIZON max_minibatch=$MAX_MINIBATCH_SIZE steps=$TOTAL_TIMESTEPS tag=$TAG"
