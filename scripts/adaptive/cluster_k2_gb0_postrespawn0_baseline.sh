...
# gb=0 respawn baseline with reward_goal_post_respawn=0 → only the FIRST
# goal-reach in the episode gives reward (1.0); all subsequent respawns
# give 0. Reward density drops from gb=0's dense stream (~10-15 events,
# total ~3.5 reward) to ~1 event/episode. This sparser than gb=3 (which has
# 2 trial-end opportunities).
#
# Diagnostic: isolates whether dense post-respawn reward is what makes gb=0
# train to 0.997. If this gb=0_pr0 still trains well → trial-structure is
# the issue, not sparsity. If it collapses to gb=3-like scores → reward
# sparsity is the dominant lever.
#
# A/B partner against:
#   - R0 (9505899_0): gb=0, post_respawn=0.25 default → 0.997
#   - T2 (9485681_2): gb=3, std GAE → 0.33
#
# Submit: sbatch scripts/adaptive/cluster_k2_gb0_postrespawn0_baseline.sh
...
TAG="baseline_k2_gb0_postrespawn0_lane0.05"

echo "[gb0_pr0] seed=$SEED  lr=$LR  ent_coef=$ENT_COEF  tag=$TAG"
...
singularity exec --nv \
 ...
 bash -c "
   ...
   xvfb-run -a puffer train puffer_adaptive_drive \
     ...
     --env.goal-behavior 0 \
     --env.reward-goal-post-respawn 0 \
