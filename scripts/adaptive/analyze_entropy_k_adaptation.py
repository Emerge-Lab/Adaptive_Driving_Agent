...
def g1(data):
    ...
    fig.tight_layout(); fig.savefig(OUT / "g1_mean_delta_heatmap.jpg", dpi=140, bbox_inches="tight")
    plt.close(fig); print("saved g1_mean_delta_heatmap.jpg")
    return M
...
def g2(data):
    ...
    for i, e in enumerate(ENTROPIES):
        for j, k in enumerate(KS):
            if k in data[e] and len(data[e][k]) == len(SEEDS):
                ...
                mean[i, j] = vals.mean(); sem[i, j] = vals.std(ddof=1) / np.sqrt(len(vals))
    fig, axes = plt.subplots(1, 2, figsize=(12, 4.8))
    # left: Δ vs entropy, one line per k
    cmap = plt.cm.viridis(np.linspace(0.1, 0.9, len(KS)))
    for j, k in enumerate(KS):
        axes[0].errorbar(ENTROPIES, mean[:, j], yerr=sem[:, j], marker="o", lw=1.5,
                         capsize=3, color=cmap[j], label=f"k={k}")
    axes[0].set_xscale("log"); axes[0].set_xticks(ENTROPIES)
    axes[0].set_xticklabels([f"{e:.2f}" for e in ENTROPIES])
    axes[0].axhline(0, color="0.7", ls=":", lw=0.8)
    axes[0].set_xlabel("co-player entropy_ub (log)"); axes[0].set_ylabel("mean per-map Δ")
    ...
    for i, e in enumerate(ENTROPIES):
        axes[1].errorbar(KS, mean[i, :], yerr=sem[i, :], marker="s", lw=1.5,
                         capsize=3, color=cmap2[i], label=f"entropy={e:.2f}")
    axes[1].axhline(0, color="0.7", ls=":", lw=0.8)
    axes[1].set_xticks(KS); axes[1].set_xlabel("adaptation trials k")
    ...
    fig.tight_layout(); fig.savefig(OUT / "g2_delta_lines.jpg", dpi=140, bbox_inches="tight")
    plt.close(fig); print("saved g2_delta_lines.jpg")
    return mean, sem
...
def g3(data):
    ...
    fig.tight_layout(); fig.savefig(OUT / "g3_net_adapt_heatmap.jpg", dpi=140, bbox_inches="tight")
    plt.close(fig); print("saved g3_net_adapt_heatmap.jpg")
    return M
...
def g4(data):
    ...
    fig.tight_layout(); fig.savefig(OUT / "g4_seed_consistency_heatmap.jpg", dpi=140, bbox_inches="tight")
    plt.close(fig); print("saved g4_seed_consistency_heatmap.jpg")
    return M
    ...
    for i, e in enumerate(ENTROPIES):
        for j, k in enumerate(KS):
            if k in data[e] and len(data[e][k]) == len(SEEDS):
                ...
                print(f"{e:>5.2f} {k:>2} {len(mids):>5} {g1M[i,j]:>+8.04f} "
                      f"{g3M[i,j]:>+8.1f}% {g4M[i,j]:>8.03f}")
...
def main():
    ...
    g1M = g1(data); g2(data); g3M = g3(data); g4M = g4(data)
