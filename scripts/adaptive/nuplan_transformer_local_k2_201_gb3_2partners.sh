...
# 2 k=2 transformer runs, gb=3 (GOAL_TRIAL) — low-entropy partners only.
# Same config as the 4-partner sweep but bigger nw (more rollout throughput
# per run) since we're only running 2 concurrently.
#
# Train on nuplan_201 (5000 maps), eval + renders on nuplan_hard (540 maps).
# 2B total timesteps each.
#
# Memory math: per-run pinned ≈ nw*2.83 GiB obs + ~10 GiB overhead.
# At nw=24: 2 × (24*2.83 + 10) ≈ 156 GiB — over 132 GiB cap (tight).
# If OOMs, drop to nw=20.
#
# Default GPUs: 0-1. Override:    GPUS="0 1" bash <script>
# Stop:        tmux kill-session -t ada_k2_gb3_2partners
...
NUM_WORKERS=16; NUM_ENVS=16   # nw=18 OOM'd at first eval epoch (~50 min); 2*(16*2.83+10)=110 GiB → 22 GiB headroom for eval/render spike
MINIBATCH_MULTIPLIER=50; MAX_MINIBATCH_SIZE=20100   # halved from 100/40200 to fit forward in 24GB VRAM (was OOMing at 22.74/23.53)
...
SESSION=${SESSION:-ada_k2_gb3_2partners}
...
for ((i=0; i<${#RUNS[@]}; i++)); do
  ...
  GPU=${GPU_ARR[$i]}
  WIN="$LABEL"
  TAG="ada_k2_gb3_${LABEL}_g${GAMMA}_lane${LANE_REWARD}_nw${NUM_WORKERS}"
...
echo 'Running $LABEL: partner=$PARTNER_ID e_ub=$EUB gamma=$GAMMA gb=3 nw=$NUM_WORKERS on GPU $GPU' && \
...
echo
echo "Launched ${#RUNS[@]} k=2 gb=3 runs (nw=$NUM_WORKERS) in tmux session '$SESSION'"
for ((i=0; i<${#RUNS[@]}; i++)); do
  read -r LABEL PARTNER_ID EUB <<< "${RUNS[$i]}"
