...
def render_videos(config, vecenv, logger, epoch, global_step, bin_path):
    ...
    try:
        ...
        if env_cfg is not None:
            ...
            if getattr(env_cfg, "reward_conditioned", False):
                base_cmd.extend(["--use-rc", "1"])
            if getattr(env_cfg, "entropy_conditioned", False):
                base_cmd.extend(["--use-ec", "1"])
            if getattr(env_cfg, "discount_conditioned", False):
                base_cmd.extend(["--use-dc", "1"])
