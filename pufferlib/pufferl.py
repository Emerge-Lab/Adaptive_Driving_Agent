...
class PuffeRL:
    def __init__(self, config, vecenv, policy, logger=None):
        torch.set_float32_matmul_precision(os.environ.get("PUFFER_FP32_MATMUL_PRECISION", "high"))
    ...
    def _fill_external_co_player_actions(self, full_obs, info, env_id, dones, truncs):
        ...
        if all("k_cache" in s and s["k_cache"] is not None for s in states):
            ...
            batched_state["k_cache"] = [torch.cat([s["k_cache"][li] for s in states], dim=0) for li in range(n_layers)]
            batched_state["v_cache"] = [torch.cat([s["v_cache"][li] for s in states], dim=0) for li in range(n_layers)]
        ...
        while self.full_rows < self.segments:
            ...
            with torch.no_grad():
                ...
                if rem_buf is None:
                    rem_buf = getattr(self.vecenv.driver_env, "removed", None)
                if rem_buf is not None:
                    rem_step = torch.as_tensor(np.asarray(rem_buf)[env_id], device=device, dtype=torch.bool)
    ...
    def train(self):
        ...
        for mb in range(self.total_minibatches):
            ...
            #   a value-chain cut.
            bootstrap_stop = (self.terminals + self.truncations + self.removed_history.float()).clamp(max=1.0)
            if _TRIAL_DEBUG_ENABLED:
                _trial_debug_log(
                    "gae_outer_pre",
                    epoch=int(self.epoch),
                    ...
                    bootstrap_overlap=float(torch.minimum(self.terminals, self.truncations).sum().item()),
            ...
            mb_bootstrap_stop = (mb_terminals + mb_truncations + mb_removed.float()).clamp(max=1.0)
            if _TRIAL_DEBUG_ENABLED:
                _trial_debug_log(
                    "gae_inner",
                    epoch=int(self.epoch),
        ...
        if _TRIAL_DEBUG_ENABLED:
            ...
            _trial_debug_log(
                ...
                explained_var=float(
                    explained_var.item() if not torch.isnan(torch.tensor(float(explained_var))) else 0.0
                ),
                cache=cache_stats,
                position=pos_stats,
            )
